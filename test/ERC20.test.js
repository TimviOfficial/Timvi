...
contract('ERC20', function ([_, owner, recipient, anotherAccount]) {
    ...
    describe('_burn', function () {
        ...
        it('rejects a null account', async function () {
            await expectRevert.unspecified(this.token.burn(ZERO_ADDRESS, 1));
        });

        describe('for a non null account', function () {
            it('rejects burning more than balance', async function () {
                await expectRevert.unspecified(this.token.burn(owner, initialSupply.add(new BN(1))));
            });
    ...
    describe('_burnFrom', function () {
        ...
        describe('for a non null account', function () {
            it('rejects burning more than allowance', async function () {
                await expectRevert.unspecified(this.token.burnFrom(owner, allowance.add(new BN(1))));
            });

            it('rejects burning more than balance', async function () {
                await expectRevert.unspecified(this.token.burnFrom(owner, initialSupply.add(new BN(1))));
            });
            ...
            const describeBurnFrom = function (description, amount) {
                describe(description, function () {
                    ...
                    it('decrements owner balance', async function () {
                        const expectedBalance = initialSupply.sub(amount);
                        expect((await this.token.balanceOf(owner))).to.be.bignumber.equal(expectedBalance);
                    });

                    it('decrements spender allowance', async function () {
                        const expectedAllowance = allowance.sub(amount);
                        expect((await this.token.allowance(owner, spender))).to.be.bignumber.equal(expectedAllowance);
                    });
